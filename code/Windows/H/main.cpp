...
int main(int argc, char **argv)
{
	...
	bool		bExit = false, bCollision = true, bKey = false, bEnd = false;
	int			AfficheLevel = 1;
	...
	while (!bExit)
	{
		...
		keys = SDL_GetKeyState(NULL);
		 ...
		 if (bEnd == false)
		 {
				...
				/*if ((keys[SDLK_DOWN])&( keys[SDLK_LEFT]))
				{	Game.CheckDoor();
					iCollision = H.GetCharac_j()-1;
					jCollision = H.GetCharac_i()+1;
					bCollision = Game.Collision(jCollision, iCollision);
